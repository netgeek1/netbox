##############
# This was an attempt to do teh same witht eh latest version of netbox and plugins, but it is currently broke.
# 20260114
#
# *************  IT IS BROKEN  *************
##############
...
# Core NetBox version (pinned)
NETBOX_VERSION="v4.4.9"

# Plugin pinned versions (authoritative)
PLUGIN_netbox-secrets="2.4.1"
PLUGIN_slurpit-netbox="1.2.7"
PLUGIN_netbox-plugin_dns="1.4.7"
PLUGIN_netbox-inventory="2.4.1"
PLUGIN_netbox-routing="0.3.1"
PLUGIN_netbox-topology_views="4.4.0"

# Install mode:
#  pinned           -> NetBox pinned, plugins pinned, Slurp'it latest
#  latest_plugins   -> NetBox pinned, plugins latest, Slurp'it latest
#  latest_all       -> NetBox latest, plugins latest, Slurp'it latest
INSTALL_MODE="pinned"
...
resolve_slurpit_version() {
    # Slurp'it is always latest in your model
    echo "latest"
}

resolve_plugin_version() {
    local plugin="$1"
    local pinned_var="PLUGIN_${plugin}"

    case "$INSTALL_MODE" in
        pinned)
            echo "${!pinned_var}"
            ;;
        latest_plugins|latest_all)
            echo "latest"
            ;;
        *)
            echo "${!pinned_var}"
...
write_plugin_req() {
    local pkg="$1"
    local version
    version="$(resolve_plugin_version "$pkg")"
...
{
    write_plugin_req "netbox-secrets"
    write_plugin_req "slurpit_netbox"
    write_plugin_req "netbox-plugin-dns"
    write_plugin_req "netbox-inventory"
    write_plugin_req "netbox-routing"
    write_plugin_req "netbox-topology-views"
...
create_plugin_dockerfile() {
    ...
    local resolved_netbox_version
    resolved_netbox_version="$(resolve_netbox_version)"

    log "Writing Dockerfile-plugins using NetBox version ${resolved_netbox_version} (mode: ${INSTALL_MODE})..."

    cat > Dockerfile-plugins <<EOF
FROM netboxcommunity/netbox:${resolved_netbox_version}
...
#    echo "  $versions"
    ...
    echo "NETBOX VERSION (target): $(resolve_netbox_version)"
    echo "PLUGIN TARGET VERSIONS:"
    for p in netbox_secrets slurpit_netbox netbox_plugin_dns netbox_inventory netbox_routing netbox_topology_views; do
        echo "  $p: $(resolve_plugin_version "$p")"
    done
...
18) Set Install Mode (Pinned / Latest Plugins / Latest Everything)
0) Exit
EOF
}
  ...
  while true; do
    ...
    case "$choice" in
      ...
      18)
          ...
          echo "1) Pinned (NetBox pinned, plugins pinned, Slurp'it latest)"
          echo "2) Latest Plugins (NetBox pinned, plugins latest, Slurp'it latest)"
          echo "3) Latest Everything (NetBox latest, plugins latest, Slurp'it latest)"
          read -rp "> " mode
      
          case "$mode" in
              1) INSTALL_MODE="pinned" ;;
              2) INSTALL_MODE="latest_plugins" ;;
              3) INSTALL_MODE="latest_all" ;;
              *) echo "Invalid selection." ;;
          esac
          ;;
      0) exit 0 ;;
