...
#     NetBox 4.4.0+ handles housekeeping internally -- including it caused
#     "invalid compose project" errors)
#
#  Changelog v2.0.4:
#   - DOCKER_COMPOSE global correctly initialised (was self-referential after
#     regex pass; caused "unbound variable" error on every run)
#   - Added detect_docker_compose(): tries compose plugin then legacy binary
#   - Added auto-sudo: check_root() re-execs with sudo when not root
#   - install_deps: Docker installed from official Docker apt repo for
#     reliable compose v2 plugin support; fallback to docker.io
#   - install_deps: download-mibs guarded with cmd_exists check
#   - install_deps: pip packages installed per-package with || true
#   - deploy_netbox: pre-generate API token; store before wait loop
#   - docker-compose.override.yml: removed obsolete "version:" key and
#     removed netbox-housekeeping service
#   - Startup readiness: replaced curl -sf (fails on 403) with HTTP-code check
#     accepting 2xx/3xx/4xx (any response means NetBox is alive)
#   - scan_all_hosts: use fd3 for host-file loop so background probe
#     subprocesses cannot consume the while-loop stdin
#   - All background probes get </dev/null to prevent accidental stdin reads
#   - nb_upsert_device: validate dtype_id/role_id/site_id before --argjson
#
#  Changelog v2.0.5:
#   - Admin password saved to NETBOX_ADMIN_PASS in config; shown in menu
#   - pip: added --root-user-action=ignore (suppresses venv warning)
#   - docker-compose.override.yml: PROCESSES:"2" (suppresses gunicorn warning)
#   - nmap: removed U: UDP ports (need -sU); removed ftp-banner (dropped in
#     nmap 7.90+); removed telnet-ntlm-info, snmp-sysdescr, snmp-interfaces
#     (not valid script names); removed -sC (ran broken default scripts)
#   - sync_to_netbox: two-step reachability check -- TCP port nc test first,
#     then HTTP code check; 401/403 prompts for token correction inline
#
#  Changelog v2.0.6:
#   - Credentials file written BEFORE the startup wait loop so it always
#     exists even when NetBox times out during deploy
#   - docker-compose.override.yml: all env values quoted; added
#     SUPERUSER_API_KEY as alias alongside SUPERUSER_API_TOKEN
#   - Startup readiness loop no longer exits on timeout -- prints guidance
#     and breaks instead of returning 1
#   - Admin/token setup moved to reliable Django shell approach using
#     set_password() + Token.objects.get_or_create(key=<pre-generated>)
#   - Admin password shown in NetBox Management menu
#
#  Changelog v2.0.7:
#   - ALL log functions (log_info/warn/ok/step/debug) now write display
#     output to stderr (>&2).  This was the root cause of ALL jq --argjson
#     "invalid JSON" errors: log text was captured inside $(...) alongside
#     numeric IDs and passed to --argjson, corrupting the payload.
...
#     (Cisco vtpVlanName 9.9.46.1.3.1.1.2); all parsed and stored in
#     snmp.json as ip_table, vlan_pvid, vlan_names; mac_port_map enriched
#     with if_name, VLAN, and remote_ip from ARP table
#   - merge_host_data: ip_table, vlan_pvid, vlan_names passed through to
#     host JSON so sync_to_netbox can use them
#   - sync_to_netbox: each SNMP interface now gets its IP from ip_table
#     assigned in NetBox; VLAN from vlan_pvid created and set as
#     untagged_vlan with mode=access; CDP/LLDP neighbours create cable
#     connections between devices in NetBox; interface descriptions
#     enriched with LLDP/CDP neighbour info
#   - Classification: added snmp_up flag (True when SNMP responded);
#     Switch/Router detection uses (snmp_up OR port 161 open) removing
#     the hard dependency on nmap seeing TCP 161; added switch keywords:
#     powerconnect, 1810g, 1910, 2530, 2920, 2960, 3750, 3850, 9300,
#     netgear gs, netscreen, vyos; ProCurve correctly classified as Switch
#   - Subnet filtering: after host discovery, IPs outside the requested
#     CIDR are removed (prevents docker bridge / host IPs from leaking in)
#   - map_switchports: full rewrite -- shows all interfaces with admin/oper
#     status, speed, VLAN, MAC, remote IPs; tries Cisco per-VLAN community
#     strings (community@vlan) when standard bridge MIB returns 0 MACs;
#     results saved to JSON in discovery dir
#   - Syntax verified clean; 0 non-ASCII characters; all changelogs retained
# =============================================================================
...
DOCKER_COMPOSE="docker compose"    # overridden by detect_docker_compose()
...
# KEY: all display output uses >&2 so log calls are never captured inside $(...)
log_info()  { _log "INFO"  "$@"; echo -e "${G}[INFO]${NC}  $*" >&2; }
log_warn()  { _log "WARN"  "$@"; echo -e "${Y}[WARN]${NC}  $*" >&2; }
log_error() { _log "ERROR" "$@"; echo -e "${R}[ERROR]${NC} $*" >&2; }
log_ok()    { _log "OK"    "$@"; echo -e "${G}[OK]${NC}    $*" >&2; }
log_debug() { _log "DEBUG" "$@"; [[ $DEBUG_MODE -eq 1 ]] && echo -e "${D}[DEBUG]${NC} $*" >&2; }
...
DCEOF
    ...
    # set_password() hashes the password properly; env var injection is unreliable.
    log_info "Configuring admin credentials via Django shell (may retry ~90s)..."
    local admin_setup_py admin_setup_result admin_setup_tries=0
    admin_setup_py="from django.contrib.auth.models import User\n"
    admin_setup_py+="from users.models import Token\n"
    admin_setup_py+="u,_=User.objects.get_or_create(username='admin')\n"
    admin_setup_py+="u.set_password('${admin_pass}')\n"
    admin_setup_py+="u.is_superuser=True; u.is_staff=True; u.save()\n"
    admin_setup_py+="t,_=Token.objects.get_or_create(user=u,key='${api_token}')\n"
    admin_setup_py+="print('SETUP_OK:'+t.key)"
    until admin_setup_result=$(cd "$NETBOX_DIR" && \
            ${DOCKER_COMPOSE} exec -T netbox \
            python manage.py shell -c "$admin_setup_py" 2>/dev/null \
            | grep "^SETUP_OK:"); do
        sleep 5; (( admin_setup_tries++ )) || true
        if (( admin_setup_tries > 18 )); then
            log_warn "Django shell timed out -- password/token may not be set"
            log_warn "Manual fix: docker exec -it <netbox> python manage.py changepassword admin"
            break
        fi
    done
    if [[ "$admin_setup_result" == SETUP_OK:* ]]; then
        NETBOX_API_TOKEN="${admin_setup_result#SETUP_OK:}"
        log_ok "Admin + token configured: ${NETBOX_API_TOKEN:0:12}..."
        save_config
        ...
        log_warn "Django shell setup incomplete -- check credentials file"
    fi
...
nb_get_or_create_vlan() {
    ...
    if [[ -z "$id" ]]; then
        local payload
        payload=$(jq -n --argjson vid "$vid" --arg name "$name" \
...
discover_live_hosts() {
    ...
    # Remove IPs outside the requested target CIDR.
    # Prevents docker bridge, host interface IPs leaking in from ARP cache.
...
try:
    ...
    pass  # keep file unchanged on any error
" "$target" "$LIVE_HOSTS_FILE" 2>/dev/null || true
    fi
...
probe_snmp() {
    ...
    local ip_table_raw pvid_raw vlan_name_raw bport_raw
    ifaces_raw=$(  _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.2.2)
    mac_table=$(   _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.17.4.3.1)
    bport_raw=$(   _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.17.1.4.1.2)
    arp_table=$(   _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.4.22.1)
    ip_table_raw=$(  _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.4.20.1)
    pvid_raw=$(    _snmp_walk "$ip" "$t" "$tout" 1.3.6.1.2.1.17.7.1.4.5.1.1)
    vlan_name_raw=$(_snmp_walk "$ip" "$t" "$tout" 1.3.6.1.4.1.9.9.46.1.3.1.1.2)
    ...
    echo "$bport_raw"     > "$tmp/snmp_bport.txt"
    echo "$arp_table"     > "$tmp/snmp_arp.txt"
    echo "$ip_table_raw"  > "$tmp/snmp_iptable.txt"
    echo "$pvid_raw"      > "$tmp/snmp_pvid.txt"
    echo "$vlan_name_raw" > "$tmp/snmp_vlannames.txt"
...
bport_raw      = read_file('snmp_bport.txt')
arp_table_raw  = read_file('snmp_arp.txt')
ip_table_raw   = read_file('snmp_iptable.txt')
pvid_raw       = read_file('snmp_pvid.txt')
vlan_name_raw  = read_file('snmp_vlannames.txt')
...
# Build ARP lookup mac->ip for enriching mac_port_map remote_ip
arp_mac_to_ip = {}
for line in arp_table_raw.split('\n'):
    m = re.match(r'.*4\.22\.1\.2\.(\d+)\.(\d+\.\d+\.\d+\.\d+)\s*=\s*INTEGER:\s*(\d+)', line)
    if m: arp_entries  # already done above
for line in arp_table_raw.split('\n'):
    m_ip  = re.search(r'\.(\d+\.\d+\.\d+\.\d+)\s*=', line)
    m_mac = re.search(r'=\s*(?:STRING|Hex-STRING):\s*([0-9a-fA-F: ]+)', line)
    if m_ip and m_mac and '4.22.1.2' in line:
        arp_mac_to_ip[m_mac.group(1).strip()] = m_ip.group(1)

# IP address table: ip -> {if_index, netmask}
ip_table = []
ip_if_map = {}   # ip -> if_index
ip_mask_map = {} # ip -> netmask
...
for ip_addr, if_idx in ip_if_map.items():
    ip_table.append({'ip': ip_addr,
                      'if_index': if_idx,
                      'mask': ip_mask_map.get(ip_addr, '255.255.255.0')})

# VLAN PVID (dot1qPvid): bridge_port -> vlan_id
vlan_pvid = {}
...
for line in vlan_name_raw.split('\n'):
    ...
    if m: vlan_names[m.group(1)] = m.group(2).strip().strip('"').strip()

# Enrich mac_port_map with VLAN and remote IP
for entry in mac_port_map:
    bp = entry['port_index']
    ii = entry.get('if_index', bp)
    entry['vlan']      = vlan_pvid.get(bp, vlan_pvid.get(ii, ''))
    entry['vlan_name'] = vlan_names.get(entry['vlan'], '')
    entry['remote_ip'] = ''
    for arp_e in arp_entries:
        if arp_e['if_index'] == ii:
            entry['remote_ip'] = arp_e['ip']; break
...
# snmp_up: SNMP responded -- reliable indicator this is a managed device
snmp_up = bool(snmp.get('available'))

FIREWALL = ['firewall','fortigate','fortios','palo alto','checkpoint','asa','sonicwall','opnsense','pfsense','netscreen']
ROUTER   = ['router','gateway','ios xe','ios xr','junos','routeros','vyos ',' gw ']
SWITCH   = ['switch','catalyst','nexus',' eos ','comware','procurve','arubaos','ex series','qfx',
            'powerconnect','1810g','1910','2530','2920','2960','3750','3850','9300','netgear gs']
AP       = ['access point','aironet','unifi','airmax','lightweight ap','thin ap']
SERVER   = ['linux','ubuntu','debian','centos','rhel','windows server','esxi','vmware','proxmox','freebsd']
PRINTER  = ['printer','jetdirect','xerox','ricoh','canon','brother','lexmark']
...
def get_oid(oid, comm=None):
    c = comm or community
    try:
        r = subprocess.run(
            ['snmpget', '-v2c', '-c', c, '-t', timeout, '-r1', ip, oid],
...
if_names   = {}
if_status  = {}   # oper status
if_admin   = {}   # admin status
if_speed   = {}
if_mac     = {}
...
# Also try standard dot1qVlanStaticName
if not vlan_names:
...
mac_to_port = {}
raw_mac = walk('1.3.6.1.2.1.17.4.3.1.2')
for line in raw_mac.split('
'):
    m = re.match(
        r'.*17\.4\.3\.1\.2\.(\d+\.\d+\.\d+\.\d+\.\d+\.\d+)\s*=\s*INTEGER:\s*(\d+)', line)
    if m:
...
# If no MACs found, try Cisco per-VLAN community strings (community@vlan)
if not mac_to_port and vlan_names:
    print(f'  Standard bridge MIB returned 0 MACs -- trying per-VLAN communities...')
    for vid in list(vlan_names.keys())[:10]:  # try first 10 VLANs
        vlan_comm = f'{community}@{vid}'
        raw = walk('1.3.6.1.2.1.17.4.3.1.2', comm=vlan_comm)
...
'):
            ...
            if m:
                ...
                mac_to_port[mac] = m.group(2)
        if mac_to_port:
            print(f'  Found MACs via VLAN community @{vid}')
            break

print(f'  Found {len(mac_to_port)} MAC table entries')

# ARP: IP reverse lookup
arp_map = {}  # mac-like string -> ip
...
# Build per-interface entry list
# First: all interfaces (with or without MACs)
if_entries = {}
for idx, name in if_names.items():
    ...
    for mac in macs_on_port:
        for k, v in arp_map.items():
            if mac.replace(':', '') in k.replace(':', '').replace(' ', ''):
    ...
    if_entries[idx] = {
        ...
        'speed_mbps': str(int(if_speed.get(idx, 0)) // 1000000) if if_speed.get(idx, '0').isdigit() else '?',
        'mac':        if_mac.get(idx, ''),
        'vlan':       vlan,
...
if vlan_names:
    print(f'  VLANs      : ' + ', '.join(f'{k}={v}' for k,v in sorted(vlan_names.items(), key=lambda x:int(x[0]) if x[0].isdigit() else 0)[:10]))
print()
# Header
print('  {:<24} {:<5} {:<5} {:<8} {:<6} {:<18} {:<16} {}'.format(
    'Interface', 'Adm', 'Oper', 'Speed', 'VLAN', 'VLAN Name', 'MAC/OUI', 'Remote IPs / Client MACs'))
print('  ' + '-' * 110)
for e in port_entries:
    clients_str = ', '.join(e['remote_ips']) if e['remote_ips'] else ', '.join(e['clients'][:3])
    print('  {:<24} {:<5} {:<5} {:<8} {:<6} {:<18} {:<16} {}'.format(
        e['if_name'][:23],
        e['admin'][:4],
        e['oper'][:4],
...
sync_to_netbox() {
    ...
    while IFS= read -r host; do
        ...
        if [[ -n "$dev_id" && "$dev_id" != "null" ]]; then
            ...
            # Gather SNMP supplementary data for this device
            local ip_table_json vlan_pvid_json vlan_names_json
            local lldp_json cdp_json
            ...
            # Add SNMP interfaces with IPs, VLANs, and neighbor descriptions
            local iface
            while IFS= read -r iface; do
                local if_name if_mac if_type if_idx nb_type if_desc lldp_desc cdp_desc
                ...
                # Description from LLDP/CDP neighbors on this port
                lldp_desc=$(echo "$lldp_json" | jq -r \
                    "[.[] | select(.port_id==\"$if_name\" or .port_desc==\"$if_name\")
                      | \"LLDP: \"+.sys_name] | join(\"; \")" 2>/dev/null || echo "")
                cdp_desc=$(echo "$cdp_json" | jq -r \
                    "[.[] | select(.remote_port==\"$if_name\")
                      | \"CDP: \"+.device_id] | join(\"; \")" 2>/dev/null || echo "")
                if_desc="${lldp_desc}${cdp_desc:+; $cdp_desc}"
                ...
                # Assign IP from SNMP IP address table to this interface
...
print(ipaddress.IPv4Network(\"$iface_ip/$iface_mask\",strict=False).prefixlen)" \
                ...
                # Assign VLAN (dot1qPvid) to interface
                ...
                if [[ -n "$pvid" && "$pvid" =~ ^[0-9]+$ && "$pvid" != "0" ]]; then
                    ...
                    vlan_id=$(nb_get_or_create_vlan "$pvid" "$vlan_nm" "$site_id" \
                        2>/dev/null) || true
            ...
            # Create CDP/LLDP topology cables between discovered neighbours
            local nbr
            while IFS= read -r nbr; do
                local nbr_name nbr_local_port nbr_remote_port nbr_enc nbr_dev_id
                ...
                nbr_if_id=$(nb_add_interface "$nbr_dev_id" \
                    "${nbr_remote_port:-to-$hostname}" "other" "" \
                    "Topology link to $hostname" 2>/dev/null) || true
                ...
                log_info "Cable: $hostname <-> $nbr_name"
                        ...
                        echo "$host" | jq -c '.cdp_neighbors[]?' 2>/dev/null; } 2>/dev/null || true)
